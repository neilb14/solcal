...
def local_hour_angle(time,latitude,zenith)
	...
	longitude_sun = adjust(anomaly + to_deg(1.916 * Math.sin(to_rad(anomaly))) + (0.020*Math.sin(2*to_rad(anomaly))) + 282.634, 360)
	right_ascension = adjust(to_deg(Math.atan(0.91764 * Math.tan(to_rad(longitude_sun)))), 360)
	...
	decension_sin = 0.39782*Math.sin(to_rad(longitude_sun))
	decension_cos = Math.cos(Math.asin(decension_sin))
	result = to_deg(Math.cos(to_rad(zenith)) - (decension_sin*Math.sin(latitude)))/(decension_cos*Math.cos(latitude))
	raise "Sun never rises at this location on this date [#{result}]" if result > 1
	raise "Sun never sets at this location on this date [#{result}]" if result < -1
...
local_hour_angle_rising = local_hour_angle(time_rising,to_rad(latitude),zenith)
local_hour_angle_setting = local_hour_angle(time_setting,to_rad(latitude),zenith)
